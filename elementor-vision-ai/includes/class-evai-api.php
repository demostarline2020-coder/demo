...
class API {
    ...
    private static function analysis_pass_prompts(array $capabilities): array {
        ...
        return [
            [
                'key' => 'visual_block_detection',
                'label' => 'visual block detection',
                'prompt' => 'Step 1: Visual Block Detection. Start like a designer looking at an unknown screenshot, not like a marketing-page template. Do not classify sections, do not name blocks as hero/services/footer, and do not output Elementor JSON. Split the screenshot into visual regions using only visible boundaries, changes in background, whitespace, alignment, and content grouping. Return JSON only: {"canvas":{"width":null,"height":null},"blocks":[{"block_id":"block_1","x_start":null,"y_start":null,"x_end":null,"y_end":null,"boundary_reason":"","dominant_visual_weight":"low|medium|high","observed_elements":[],"background_observation":"","notes":""}],"confidence":{"layout":0.0}}. Every coordinate, boundary, and observation must come from the image itself.',
            ],
            [
                'key' => 'visual_structure_analysis',
                'label' => 'visual structure analysis',
                'prompt' => 'Step 2: Visual Structure Analysis. Use the detected blocks as the source of truth. For every block, describe the design structure only: containers, rows, columns, nested groupings, alignment patterns, image/icon/CTA areas, visual hierarchy, and spacing relationships. Do not classify section type. Do not choose Elementor widgets yet. Return JSON only: {"blocks":[{"block_id":"","container_count":null,"nested_container_count":null,"rows":null,"columns":null,"column_ratios":[],"content_groupings":[],"alignment_pattern":"","visual_hierarchy":[],"image_regions":[],"icon_regions":[],"cta_regions":[],"spacing_relationships":[],"notes":""}],"confidence":{"layout":0.0,"spacing":0.0}}. If a block is one column, return one column; if ratios are unclear, explain that in notes instead of guessing a default.',
            ],
            [
                'key' => 'section_classification',
                'label' => 'section classification after observation',
                'prompt' => 'Step 3: Section Classification. Only now classify each visual block, based on the visual evidence from Steps 1 and 2. The discovered_type may be any plain-English role such as navigation, intro, product grid, dashboard area, menu list, gallery, article, pricing, testimonial, checkout, footer, or unknown. Also provide builder_type only as a PHP builder hint using one of: header, hero, stats, features, services, cta, footer, content. Use content when no builder type is a strong visual fit. Return JSON only: {"blocks":[{"block_id":"","discovered_type":"","builder_type":"content","classification_confidence":0.0,"visual_evidence":"","notes":""}],"confidence":{"layout":0.0}}. Do not force every page into predefined marketing sections.',
            ],
            [
                'key' => 'widget_reasoning',
                'label' => 'Elementor expert widget reasoning',
                'prompt' => 'Step 4: Widget Reasoning. Think like an Elementor expert implementing each observed visual element. ' . $capabilityContext . ' For every block and observed element, explain the best Elementor implementation using only available widgets or fallback compositions. Reason about overlays, play buttons, menus, forms, product/card groups, icon areas, image areas, repeated lists, absolute-like overlays, borders, radius, and spacing. Return JSON only: {"blocks":[{"block_id":"","decisions":[{"visual_element":"","appearance_reasoning":"","preferred_widget":"","fallback_widget":"","composition":[],"positioning_notes":"","style_notes":""}]}],"confidence":{"layout":0.0}}. Do not fill generic widget slots; create decisions only for elements actually visible in the screenshot.',
            ],
            [
                'key' => 'design_system_extraction',
                'label' => 'design system extraction',
                'prompt' => 'Step 5: Design System Extraction. Extract the design system from the screenshot. Return JSON only: {"typography":{"font_family":"","levels":[{"role":"","size":null,"weight":null,"line_height":null,"letter_spacing":null,"case":""}]},"spacing":{"section_spacing":[],"container_spacing":[],"card_spacing":[],"gaps":[]},"colors":{"primary":null,"secondary":null,"backgrounds":[],"text":[],"accents":[]},"effects":{"borders":[],"radius":[],"shadows":[],"overlays":[]},"confidence":{"typography":0.0,"spacing":0.0,"colors":0.0}}. Extract values from visible evidence. Do not invent a design system or use theme defaults.',
            ],
            [
    ...
    private static function elementor_template_prompt(array $capabilities): string {
        ...
        return $capabilityContext . 'Step 6: Complete Design Specification. Synthesize the prior visual block detection, structure analysis, classification, widget reasoning, and design system into a builder-ready DESIGN SPECIFICATION JSON. Do not output Elementor JSON or Elementor internal settings. Do not create sections from a preset list. The sections array must follow the detected visual blocks in screenshot order and should stay one-to-one with blocks unless visual fidelity requires splitting or merging. Schema with placeholders to fill from screenshot-derived observations: {"confidence":{"layout":0.0,"typography":0.0,"spacing":0.0,"colors":0.0},"sections":[{"source_block_id":"","type":"free-form discovered visual role","builder_type":"header|hero|stats|features|services|cta|footer|content","layout":"1-column|2-column|3-column|grid","heading":"","subheading":"","text":"","buttons":[],"background_color":null,"text_color":null,"accent_color":null,"card_background_color":null,"image_background_color":null,"padding_top":null,"padding_right":null,"padding_bottom":null,"padding_left":null,"gap":null,"column_gap":null,"heading_size":null,"heading_weight":null,"body_size":null,"alignment":"left|center|right","column_widths":[],"image_position":"left|right|background|none","image_height":null,"button_style":"filled|outline|text","button_radius":null,"border_radius":null,"shadow":"none|soft|medium|strong","widgets":[{"visual_element":"","widget":"heading|text-editor|image|icon-box|image-box|button|html|container|spacer|divider|social-icons|video|counter|form|nav-menu|loop-grid|slides|popup|price-table|posts","fallback_widget":"html|button|image|icon-box|container_cards","reason":""}],"items":[{"title":"","text":"","value":""}]}]}. Use builder_type only to help PHP choose a compatible generator; keep type as the discovered visual role. Max 10 sections, max 10 items per section. Confidence values must be 0 to 1. No markdown.';
    }
    ...
    private static function build_section_from_design_spec(array $section): array {
        $type = self::builder_section_type($section);
        return match ($type) {
    ...
    private static function builder_section_type(array $section): string {
        $candidate = sanitize_key(strval($section['builder_type'] ?? $section['type'] ?? 'content'));
        $aliases = [
            'navigation' => 'header',
            'nav' => 'header',
            'intro' => 'hero',
        ...
        return in_array($candidate, ['header', 'hero', 'stats', 'features', 'services', 'cta', 'footer', 'content'], true) ? $candidate : 'content';
    }
